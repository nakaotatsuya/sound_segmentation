...
def stft_(root, save_dir):
    label = "vacuum"
    save_dir = osp.join(save_dir, label)
    data_folders = osp.join(root, label)
    filelist = os.listdir(data_folders)

    for filename in filelist:
        waveform, fs = sf.read(osp.join(data_folders, filename))
        print(waveform.shape)
        #print(fs) #16000
        #print(waveform.T[:8].shape) #24000
        _, _, stft = signal.stft(x=waveform.T, fs=fs, nperseg=512, return_onesided=False)
        stft = stft[:,:96]
        cv_true = abs(stft[:256])

        cv_true = normalize(cv_true)

        cv_true = cv_true[::-1,:]
        cv_true = (cv_true * 255).astype(np.uint8)
        file_num = len(os.listdir(save_dir)) + 1
        print(file_num)
        file_name = osp.join(save_dir, "{:0=5d}.png".format(file_num))
        print(file_name)
        Image.fromarray(cv_true).save(file_name)
...
if __name__ == "__main__":
    ...
    root = osp.join(rospack.get_path("sound_segmentation"), "house_audios", "wav_house2")

    save_dir = osp.join(rospack.get_path("sound_segmentation"), "house_audios", "cv_wav_house2")
    stft_(root,save_dir)
