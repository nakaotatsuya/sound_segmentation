...
class Create16Wave():
    ...
    def timer_cb(self, timer):
        ...
        if not self.in_sound:
            ...
            kettle_probs = np.array([])

            check_fridges = np.array([])
            check_kettles = np.array([])
            check_alls = np.array([])
            ...
            for i in range(8):
                ...
                check_fridge = np.average(max_cv_pred_[175:178])
                check_fridges = np.append(check_fridges, check_fridge)

                check_all = np.average(max_cv_pred_)
                check_alls = np.append(check_alls, check_all)

                check_kettle = np.average(max_cv_pred_[50:130])
                check_kettles = np.append(check_kettles, check_kettle)

                #classification #############
                # bgr = img_jet(max_cv_pred_)
                # bgr = skimage.transform.resize(
                #     bgr, (self.insize, self.insize), preserve_range=True)
                # input_msg = self.bridge.cv2_to_imgmsg(bgr.astype(np.uint8), encoding="bgr8")
                # input_msg.header = self.header
                ...
                #     if not (j in swap_labels):
                #         rospy.logerr("Wrong target_names is given by rosparam.")
                #         exit()
                # y = self.c_model(x_data)
                # proba = y.to("cpu")[0]
                # proba_swapped = [proba[swap_labels[k]] for k, p in enumerate(proba)]
                ...
                # cls_msgs.append(cls_msg)

                # #clap_idx = cls_msg.target_names.index("clap\n")
                # #kettle_idx = cls_msg.target_names.index("kettle\n")
                # fridge_idx = cls_msg.target_names.index("fridge\n")

                # #clap_prob = cls_msgs[i].probabilities[clap_idx]
                # #kettle_prob = cls_msgs[i].probabilities[kettle_idx]
                # fridge_prob = cls_msgs[i].probabilities[fridge_idx]

                # class_label = cls_msgs[i].label_names[0]

                # #clap_prob = clap_prob.detach().numpy()
                # #clap_probs = np.append(clap_probs, clap_prob)

                # fridge_prob = fridge_prob.detach().numpy()
                # fridge_probs = np.append(fridge_probs, fridge_prob)
                ...
                #if save mode
                if save_mode:
                    image_save_dir = osp.join(self.target_dir, "{}".format(i))
                    if not osp.exists(image_save_dir):
                        makedirs(image_save_dir)
                    file_num = len(listdir(image_save_dir))
                    file_name = osp.join(
                        image_save_dir, "{:0=5d}.png".format(file_num))
                    Image_.fromarray(max_cv_pred_).save(file_name)
            ...
            if pred_spectrograms:
                #print(class_labels)
                # self.pub_cls0.publish(cls_msgs[0])
                # self.pub_cls1.publish(cls_msgs[1])
                # self.pub_cls2.publish(cls_msgs[2])
                # self.pub_cls3.publish(cls_msgs[3])
                # self.pub_cls4.publish(cls_msgs[4])
                # self.pub_cls5.publish(cls_msgs[5])
                # self.pub_cls6.publish(cls_msgs[6])
                # self.pub_cls7.publish(cls_msgs[7])

                #check kettle max
                kettle_max_idx = np.argmax(check_kettles)
                kettle_max = np.max(check_kettles)

                if kettle_max > 60:
                    print(kettle_max_idx)
                    ...
                    if LooseVersion(cv2.__version__).version[0] < 3:
                        ...
                        line_type = cv2.LINE_AA
                    max_cv_preds[kettle_max_idx] = cv2.putText(max_cv_preds[kettle_max_idx], cls_title, (0, 35),
                                                               cv2.FONT_HERSHEY_PLAIN,
                                                               1.4, (255, 255, 255), 2,
                                                               line_type)
                    pred_spectrograms[kettle_max_idx] = self.bridge.cv2_to_imgmsg(max_cv_preds[kettle_max_idx], "bgr8")

                    max_cls_msg = ClassificationResult(
                        header = self.header,
                        label_names = ["kettle"],
                        label_proba = [kettle_max_idx * 45])
                    self.pub_max_cls_kettle.publish(max_cls_msg)
                ...
                fridge_max_idx = np.argmax(check_fridges)
                fridge_max = np.max(check_fridges)
                if fridge_max > 40 and check_alls[fridge_max_idx] < 30:
                ...
                #         else:  # for opencv version > 3
                #             line_type = cv2.LINE_AA
                #         max_cv_preds[i] = cv2.putText(max_cv_preds[i], cls_title, (0, 35),
                #                                                    cv2.FONT_HERSHEY_PLAIN,
                #                                                    1.4, (255, 255, 255), 2,
                #                                                    line_type)
                #         pred_spectrograms[i] = self.bridge.cv2_to_imgmsg(max_cv_preds[i], "bgr8")
