...
class SoundSegmentationDataset2(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #mixture_phase = torch.from_numpy(mixture_phase).float()
...
if __name__ == "__main__":
    ...
    loader = DataLoader(ssd, batch_size=1, num_workers=16, shuffle=False, pin_memory=True)
    ...
    for i, (images, labels, phase) in enumerate(loader):
        ...
        print(labels.shape)
        print(phase.shape)
        np.save(osp.join(root, "save", "images_{:0=5d}".format(i+1)), images)
        np.save(osp.join(root, "save", "labels_{:0=5d}".format(i+1)), labels)
        np.save(osp.join(root, "save", "phase_{:0=5d}".format(i+1)), phase)
        #print(labels.shape)
        #print(phase.shape)
        #sys.exit()
